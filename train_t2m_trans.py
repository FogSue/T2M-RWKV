...
trans_encoder = trans.Text2Motion_Transformer(num_vq=args.nb_code, 
                                ...
                                drp_rate=args.drop_out_rate)
# if args.resume_trans is not None:
#     print ('loading transformer checkpoint from {}'.format(args.resume_trans))
#     ckpt = torch.load(args.resume_trans, map_location='cpu')
#     trans_encoder.load_state_dict(ckpt['trans'], strict=True)
# else:
#     m = trans.init_weights(trans_encoder, args.embed_dim_gpt, args.num_layers, args.nb_code+2)
#     trans_encoder.load_state_dict(m, strict=True)
# if m.keys() == trans_encoder.state_dict().keys():
#     print ("keys are identcial!")
#     for k in m.keys():
#             v1, v2 = m[k], trans_encoder.state_dict()[k]
#             if not torch.equal(v1, v2):
#                 print(False)
#                 continue
for n, p in trans_encoder.named_parameters():
...
VQVAE_params = sum(p.numel() for p in net.parameters()) / float(1e6)
RWKV_params = sum(p.numel() for p in trans_encoder.parameters()) / float(1e6)
logger.info(f"Total Parameters of VQVAE: {VQVAE_params}M")
logger.info(f"Total Parameters of RWKV: {RWKV_params}M")
...
train_loader_iter = dataset_TM_train.cycle(train_loader) # init a infinte iterator
...
wandb.init(
            project = 't2m-rwkv', 
            name = f"{args.exp_name}_L{args.num_layers}_D{args.embed_dim_gpt}_lr{args.lr}_drp{args.drop_out_rate}"
           )
wandb.define_metric('Fid', summary='min')
wandb.define_metric('Top-1', summary='max')
while nb_iter <= args.total_iter:
    
    batch = next(train_loader_iter) # one iteration
    clip_text, m_tokens, m_tokens_len = batch
    m_tokens, m_tokens_len = m_tokens.cuda(), m_tokens_len.cuda()
    bs = m_tokens.shape[0]
    target = m_tokens    # (bs, 51) 512 is end token for generation, 513 is padding token for length alignment
    target = target.cuda()
    text = clip.tokenize(clip_text, truncate=True).cuda() # text的token sequence 统一填充到77
    ...
    logits = trans_encoder(a_indices, feat_clip_text) # (B, N, 513)
    logits = logits.contiguous()

    loss_cls = 0.0
    for i in range(bs):
        # loss function         (m_tokens_len[i] + 1, 513),    (m_tokens_len[i] + 1)
        loss_cls += loss_ce(logits[i][:m_tokens_len[i] + 1], target[i][:m_tokens_len[i] + 1]) / bs

        # Accuracy
        probs = torch.softmax(logits[i][:m_tokens_len[i] + 1], dim=-1)
    ...
    # loss_cls = L2Wrap.apply(loss_cls, logits)
    ## global loss
    optimizer.zero_grad()
    loss_cls.backward()
    optimizer.step()
