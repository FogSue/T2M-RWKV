...
@torch.no_grad()        
def evaluation_transformer(out_dir, val_loader, net, trans, logger, writer, nb_iter, best_fid, best_iter, best_div, best_top1, best_top2, best_top3, best_matching, clip_model, eval_wrapper, draw = True, save = True, savegif=False, wandb_logger=None) : 
    ...
    for i in range(1):
        for batch in val_loader:
            ...
            for k in range(bs):
                try:
                    index_motion = trans.sample(feat_clip_text[k:k+1], False) # 只取前50个token
                except:
